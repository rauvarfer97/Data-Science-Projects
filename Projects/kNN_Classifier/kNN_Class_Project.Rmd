---
title: "Feature Selection and Predictive Modeling Using Weighted and Random kNN"
...
Before we begin it would be convenient to point out that we have duplicated all the tables that appear in the job so that we can format them in html and latex, using the “eval =” argument to identify which one we are using depending on whether we are rendering in .pdf or .html.

### 1. - Determination of a weighted kNN based classifier for the target variable “target” with the previously selected attributes or explanatory variables, considering the Euclidean distance. 

##### **1.1** Select the best kernel and the best k (between 1 and 30) through cross-validation.

#### **1.2** Obtain the error estimate (by cross-validation).
...
We obtained an error of approximately 130.9, which is far from ideal.

----------------------------------------------------------

<div class=text-justify>

### **2.** - Application of random kNN with cross validation. Consider the optimal k number of neighbors determined in the previous step and the Euclidean distance.

##### **2.1** Obtain the support of each predictor. Apply a predictor selection method based on that measure. 

##### **2.2** Obtain an error estimate with the attribute selection performed in the previous step.
...
### **3.** Carry out a comparative study between both results.

Regarding the values obtained for the mean squared error (MSE) of the different models we have that the model associated to the weighted kNNN gives us a better result than the random kNN model, both before and after making the selection of the optimal attributes to perform the regression. In the case of the weighted kNNN we obtain an error of 130.86, while in random we obtain 209.84 and 157.17, before and after choosing the optimal attributes respectively.
