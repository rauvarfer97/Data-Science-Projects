globals [ max-sheep max-wolves max-eagles]  ; don't let the sheep population grow too large
...
to setup
  ...
  create-eagles initial-number-eagles  ; create the wolves, then initialize their variables
  [
    set shape "hawk"
    set color red
    set size 1.5  ; easier to see
...
to go
  ...
  if not any? wolves and not any? eagles [user-message "The sheep have inherited the earth" stop ]
  ...
  ask eagles [
    ...
    eat-wolf
    death ; wolves die if they run out of energy
    reproduce-eagles ; wolves reproduce at a random rate governed by a slider
  ]
...
to eat-grass  ; sheep procedure
  ...
  if pcolor = green [
    set pcolor brown
    set energy energy + eat-grass-gain  ; sheep gain energy by eating
  ]
end
...
to reproduce-eagles  ; wolf procedure
  if random-float 100 < eagle-reproduce [  ; throw "dice" to see if you will reproduce
    set energy (energy / 2)               ; divide energy between parent and offspring
    hatch 1 [ rt random-float 360 fd 1 ]  ; hatch an offspring and move it forward 1 step
  ]
end
...
to eat-sheep  ; wolf procedure
  ...
  if prey != nobody  [                          ; did we get one? if so,
    ...
    set energy energy + eat-sheep-gain     ; get energy from eating
  ]
end

to eat-wolf  ; wolf procedure
  if random-float 100 < 50 [ let prey one-of wolves-here                   ; grab a random sheep
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [ die ]                            ; kill it, and...
...
to eat-eagle  ; wolf procedure
  if random-float 100 < 50 [ let prey one-of eagles-here                   ; grab a random sheep
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [ die ]                            ; kill it, and...
    set energy energy + eat-eagle-gain     ; get energy from eating
  ]
  ]
end
...
"eagles" 1.0 0 -2674135 true "" "plot count eagles"
